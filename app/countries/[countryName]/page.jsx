...
async function fetchData(url, options = {}) {
  ...
  const baseUrl =
    process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:3000"; // Or your actual API base
  try {
...
export default async function CountryPage({ params }) {
  // Properly await params to fix the dynamic params error
  const resolvedParams = await params;

  // Now it's safe to access the countryName property
  const decodedCountryName = decodeURIComponent(resolvedParams.countryName);
  console.log(
  ...
  console.log(
    `[CountryPage] Data received for ${decodedCountryName}:`,
