         ...
         let imageUrls = [];
          ...
          const res = await fetch(`/api/packages/${pkgId}`, {
             ...
             headers: {"Content-Type": "application/json"},
              body: JSON.stringify({title: editTitle, description: editDescription, price: Number(editPrice), durationDays: Number(editDurationDays), days: Number(editDays), nights: editNights})
          });
                 ...
                 {packages.map((pkg) => (
                      <tr key={pkg.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                           ...
                           <td className="px-6 py-4 whitespace-nowrap">
                                 {editingPackageId === pkg.id ? (
                                    <input type="number" value={editDays} onChange={(e) => setEditDays(Number(e.target.value))} className="border p-1 rounded"/>
                                   ) : (
                                       pkg.days
                                        )}
                           </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                    {editingPackageId === pkg.id ? (
                                     <input type="number" value={editNights} onChange={(e) => setEditNights(Number(e.target.value))} className="border p-1 rounded"/>
                                     ) : (
                                         pkg.nights
                                       )}
                             </td>
